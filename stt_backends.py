...
class STTWhisper(STTBase):
    ...
    def listen_once(self, secs: int = None) -> str:
        ...
        if secs is None:
            secs = self._config.whisper_secs
        ...
        try:
            ...
            mic_device = f"plughw:{self.device_index},0" if self.device_index is not None else self._config.mic_alsa
            ...
            print(f"[stt] whisper model={self._config.whisper_model_name} lang={self.lang} (INT8)")
...
class STTVosk(STTBase):
    ...
    def __init__(self, model_dir: str = None, device_index: Optional[int] = None,
        ...
        if model_dir is None:
            model_dir = self._config.get_vosk_model_path()
    ...
    def listen_once(self) -> str:
        ...
        holdms = self._config.silence_hold_ms
        maxms = self._config.max_listen_ms
...
class STTManager:
    ...
    def __init__(self, config: AppConfig | None = None):
        ...
        self.current_lang = self._config.lang_default
    ...
    def initialize_backend(self, backend_name: str, lang: str = None, device_index: Optional[int] = None) -> STTBase:
        ...
        if backend_name == "whisper":
            obj = STTWhisper(lang=lang, device_index=device_index, config=self._config)
        elif backend_name == "vosk":
            obj = STTVosk(model_dir=self._config.get_vosk_model_path(), device_index=device_index, config=self._config)
        else:
    ...
    def set_language(self, lang: str):
        ...
        if isinstance(self.current_backend, STTWhisper):
            ...
            self.current_backend = STTWhisper(lang=lang, device_index=self.current_device_index, config=self._config)
